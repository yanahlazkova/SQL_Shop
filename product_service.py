...
def display_balance_products(session):
    try:
        ...
        products_balance = (session.query(Product.id, Product.name, func.sum(ProductMovement.quantity).label('stock'))
            .join(Product)
            .group_by(Product.id)
        ...
        if products_balance:
            i = 0
            for product_id, product, stock in products_balance:
                i += 1
                # product = session.query(Product).get(product_id)
                text += f'\t\t{i}. ID: {product_id} - {product}\tзалишок: {stock}\n'
            print(text)
        else:
